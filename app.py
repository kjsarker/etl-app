...
def extract_date_from_filename(filename: str) -> str | None:
    """
    Search the filename stem for any recognised date pattern and return a
    yyyy-MM-dd string.  Returns None if nothing matches.
    """
    stem = filename.rsplit(".", 1)[0]

    S = r"(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)"
    L = r"(?:January|February|March|April|May|June|July|August|September|October|November|December)"

    # (regex, strptime_format) — most-specific first
    PATTERNS = [
    ...
    for pattern, fmt in PATTERNS:
        ...
        if m:
            ...
            for attempt in (raw, raw.title(), raw.upper()):
                try:
                    return datetime.strptime(attempt, fmt).strftime("%Y-%m-%d")
                except ValueError:
                    continue

    # 8-digit compact — use (?<!\d)/(?!\d) instead of \b because underscore is
    # a word character and \b would fail to match after "_" in names like "_20260323"
...
with right:
    ...
    if st.session_state.db_ok and df is not None:
        ...
        if add_filedate:
            ...
            if not _auto_date:
                ec2.warning("No date found in filename — enter manually below.")
            # text_input key tied to filename so it resets when a new file is uploaded
